...
{
    /* alloc_large_system_hash shrinks on low-memory systems.
     * On 2GB Chromecast, actual hashsize is 64-128 (not 1024).
     * Set to 0 as sentinel; __mm_leak will try multiple sizes. */
    futex_hashsize = 0;
}
uint32_t __futex_hash_with_mm(size_t addr, size_t mm, uint32_t hs)
{
    futex_key_t key;
    key.private.mm = (void *)mm;
    key.private.address = addr & ~KS_PAGE_MASK;
    key.private.offset = addr & KS_PAGE_MASK;
    return __futex_hash(&key, hs);
}
